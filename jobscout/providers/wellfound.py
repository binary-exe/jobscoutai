"""
Wellfound (formerly AngelList) provider.

Uses RSS feed: https://wellfound.com/jobs.rss
Note: Wellfound may require authentication for API access.
"""
...
class WellfoundProvider(Provider):
    """Provider for Wellfound (AngelList) RSS feed."""
    ...
    async def collect(
        ...
        """Collect jobs from Wellfound RSS."""
        self.reset_stats()
        ...
        if not result.ok:
            # Fallback: try alternative RSS URL
        ...
        for item in items[:criteria.max_results_per_source]:
            try:
                ...
                # Wellfound jobs are often remote
