...
Partner APIs vary by account; this adapter uses a configurable endpoint.
"""
...
DEFAULT_WHATJOBS_API_URL = "https://api.whatjobs.com/api/v1/jobs"
...
class WhatJobsProvider(Provider):
    """Provider for WhatJobs API-like endpoints."""
    ...
    async def collect(
        ...
        api_url = os.environ.get("JOBSCOUT_WHATJOBS_API_URL", DEFAULT_WHATJOBS_API_URL).strip() or DEFAULT_WHATJOBS_API_URL
        api_key = os.environ.get("JOBSCOUT_WHATJOBS_API_KEY", "").strip()
        if not api_key:
            self.log_error("JOBSCOUT_WHATJOBS_API_KEY not set")
            return []
        ...
        params = {
            ...
            "page": 1,
            "page_size": max(1, min(criteria.max_results_per_source, 100)),
        }
        ...
        for item in raw_items[: criteria.max_results_per_source]:
            ...
            try:
                job = build_job(
                    ...
                    company_paths=[("company",), ("company_name",), ("employer",)],
                    location_paths=[("location",), ("location_name",), ("city",)],
                    description_paths=[("description",), ("snippet",), ("summary",)],
                    job_url_paths=[("url",), ("job_url",), ("link",)],
                    apply_url_paths=[("apply_url",), ("application_url",), ("url",)],
                    posted_paths=[("posted_at",), ("date",), ("created_at",)],
                    employment_paths=[("employment_type",), ("contract_type",), ("type",)],
                    ...
                    tags_paths=[("tags",), ("categories",), ("skills",)],
                )
                if job:
                    jobs.append(job)
