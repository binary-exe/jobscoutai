"""
Remote.co RSS provider.

RSS feed: https://remote.co/remote-jobs/feed/
"""
...
class RemoteCoProvider(Provider):
    """Provider for Remote.co RSS feed."""
    ...
    async def collect(
        ...
        """Collect jobs from Remote.co RSS."""
        self.reset_stats()
        ...
        for item in items[:criteria.max_results_per_source]:
            try:
                ...
                # Extract company from title (format: "Company: Job Title" or "Company - Job Title")
                company = "Unknown"
                title = raw_title
                if " - " in raw_title:
